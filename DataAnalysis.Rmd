...
# Fig1a - timeline of included data (original)
``` {r, eval=F}
...
# Fig 1a v2 (plos med editorial request to remove stacked bar plots)
``` {r}
timeline_barplot <- data_NNS_sites10 %>% 
  group_by(Region, Country, Study, Year) %>% 
  count() %>% 
  mutate(loc=paste0(Country, " (",Study,")")) %>% 
  mutate(Region2=if_else(Region=="Western Africa", "W. Africa", Region)) %>%
  mutate(Region2 = factor(Region2, levels = 
                           c("South Asia", "Eastern Africa", "Southern Africa", "W. Africa"))) %>% 
  ggplot(aes(y=Year, x=loc, fill=n)) + 
  geom_tile() + 
...
``` {r}
patchwork::wrap_elements(panel=timeline_barplot) / patchwork::wrap_elements(se_region_plot) + patchwork::plot_annotation(tag_levels ="a") + patchwork::plot_layout(height=c(2,3))


patchwork::wrap_elements(se_region_plot) / patchwork::wrap_elements(panel=timeline_barplot) + patchwork::plot_annotation(tag_levels ="a") + patchwork::plot_layout(height=c(5,4))

ggsave("figures/Fig1_timelineRegionCountryStudy_siteMap.pdf", width=9.5, height=9.5)
ggsave("figures/Fig1_timelineRegionCountryStudy_siteMap.png", width=9.5, height=9.5)
ggsave("figures/Fig1_timelineRegionCountryStudy_siteMap.tiff", width=9.5, height=9.5)
...
ggsave(file="figures/AppendixFigS1.1_raw_K_country_O.tiff", width=9, height=11)
```
...
ggsave(file="figures/AppendixFigS1.2_clones_site_year.tiff", width=9, height=12)
```
...
ggsave(file="figures/AppendixFigS2.6_ClusterDaysThreshold.tiff", width=9, height=12)
...
ggsave(file="figures/AppendixFigS2.7_ClusterSNPsThreshold.tiff", width=9, height=12)
