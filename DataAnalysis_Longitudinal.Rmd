...
coverage_plot_longPre_globalRank_country_O <- longIn_cov_O$cov_table_long %>% mutate(group="All") %>%
    bind_rows(longIn_globalRank_bdCov_O$cov_table_long %>% mutate(group="Bangladesh")) %>% 
    bind_rows(longIn_globalRank_kenyaCov_O$cov_table_long %>% mutate(group="Kenya")) %>%
    bind_rows(longIn_globalRank_mlwCov_O$cov_table_long %>% mutate(group="Malawi")) %>%
    mutate(group=fct_relevel(group, c("All", "Bangladesh", "Kenya", "Malawi"))) %>%
    filter(type=="raw") %>%
    filter(rank<=10) %>%
    ggplot(aes(x=rank, y=value*100, group=group)) +
...
ggsave("figures/FigS6_Longitudinal.tiff", height=9, width=8)
```
