...
ggsave("figures/Fig2_K_global_regional_Kadj.tiff", width=8, height=6)
```
...
ggsave("figures/FigS2_K_global_regional_Kadj.tiff", width=8, height=10)
```
...
for (k in kbayes$locus_rank$locus[1:30]) {
  ...
  m <- summary(logistf(k~Year+Region, data=d, pl=FALSE))
...
region_2pc <- kbayes$region_est %>% select(locus, subgroup, mean) %>% 
  ...
  mutate(d1=abs(`Eastern Africa`-`Southern Africa`),
        ...
        d3=abs(`Eastern Africa`-`South Asia`),
        d4=abs(`Southern Africa`-`South Asia`),
        d5=abs(`Southern Africa`-`Western Africa`),
        d6=abs(`Western Africa`-`South Asia`)) %>% 
...
kbayes$region_post %>% 
    ...
    filter(rank<=30) %>%
    mutate(Region=fct_relevel(subgroup,c("Western Africa", "Southern Africa", "Eastern Africa", "South Asia"))) %>%
    ggplot() +
...
ggsave("figures/FigS3_RegionalKBayesDist.tiff", width=6, height=9)
```
...
ggsave("figures/FigS5_RegionalOBayesDist.tiff", width=6, height=5)
```
...
o_region_ST <- data_NNS_sites10 %>%
  ...
  mutate(Region=fct_relevel(Region, c("Western Africa", "Southern Africa", "Eastern Africa", "South Asia"))) %>%
...
data_NNS_sites10 %>% filter(O_genotype=="O4" & Region=="South Asia") %>% group_by(ST, K_locus) %>% count() %>% arrange(-n)
data_NNS_sites10 %>% filter(O_genotype=="O4" & Region=="South Asia") %>% group_by(ST, K_locus, Site) %>% count() %>% arrange(-n)

data_NNS_sites10 %>% filter(O_genotype=="O2a.v1") %>% group_by(Region, Study, Site, ST, K_locus) %>% count() %>% arrange(-n)
data_NNS_sites10 %>% filter(O_genotype=="O2a.v1" & Region=="South Asia") %>% group_by(ST, K_locus) %>% count() %>% arrange(-n)
data_NNS_sites10 %>% filter(O_genotype=="O2a.v1" & Region=="South Asia") %>% group_by(ST, K_locus, Site) %>% count() %>% arrange(-n)
...
kbayes_raw_coverage <- getCumulativeCoverageGlobalRegional(kbayes_raw, kbayes$locus_rank) %>%
  mutate(subgroup=fct_relevel(subgroup,c("Global", "South Asia", "Eastern Africa", "Southern Africa", "Western Africa")))
...
# add cluster-adjusted
kbayes_adj_coverage <- getCumulativeCoverageGlobalRegional(kbayes, kbayes$locus_rank) %>%
  mutate(subgroup=fct_relevel(subgroup,c("Global", "South Asia", "Eastern Africa", "Southern Africa", "Western Africa")))

coverage_rawAdj_globalTop20_globalRegional <- plotCoverageRawAdj(kbayes_raw_coverage, kbayes_adj_coverage, kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "")

coverage_rawAdj_globalTop20_globalRegional
```
...
# cluster-adjusted
kbayes_fatal_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/K_Fatal_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/K_Fatal_min10_adj_28_posterior_subgroup.csv.gz")

kbayes_fatal_adj_min10perSite_coverage <- getCumulativeCoverage(kbayes$locus_rank, kbayes_fatal_adj_min10perSite$global_post) %>% mutate(type="min10")
```
...
# cluster-adjusted
kbayes_esbl_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/K_ESBL_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/K_ESBL_min10_adj_28_posterior_subgroup.csv.gz")

kbayes_esbl_adj_min10perSite_coverage <- getCumulativeCoverage(kbayes$locus_rank, kbayes_esbl_adj_min10perSite$global_post) %>% mutate(type="min10")
```
...
# cluster adjusted
kbayes_cp_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/K_Carba_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/K_Carba_min10_adj_28_posterior_subgroup.csv.bz2")

kbayes_cp_adj_min10perSite_coverage <- getCumulativeCoverage(kbayes$locus_rank, kbayes_cp_adj_min10perSite$global_post) %>% mutate(type="min10")
```
...
# add cluster adjusted
kbayes_adj_coverage_subgroups <- kbayes_esbl_adj_min10perSite_coverage %>% mutate(subgroup="ESBL") %>%
  bind_rows(kbayes_cp_adj_min10perSite_coverage %>% mutate(subgroup="CP")) %>%
  bind_rows(kbayes_fatal_adj_min10perSite_coverage %>% mutate(subgroup="Fatal")) %>%
  bind_rows(kbayes_adj_coverage %>% filter(subgroup=="Global") %>% mutate(subgroup="All") ) %>%
  mutate(subgroup=fct_relevel(subgroup,c("All", "Fatal", "ESBL", "CP")))

coverage_rawAdj_globalTop20_subgroups <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups, kbayes_adj_coverage_subgroups, kbayes$locus_rank, maxRank=20, cols=group_cols, linetype_title = "")

```


# raw vs cluster-adjusted coverage global top20 K
``` {r}
# All
coverage_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups %>% filter(subgroup %in% c("All")), kbayes_adj_coverage_subgroups %>% filter(subgroup %in% c("All")), kbayes$locus_rank, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# Fatal
coverage_fatal_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups %>% filter(subgroup %in% c("Fatal")), kbayes_adj_coverage_subgroups %>% filter(subgroup %in% c("Fatal")), kbayes$locus_rank, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# ESBL
coverage_esbl_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups %>% filter(subgroup %in% c("ESBL")), kbayes_adj_coverage_subgroups %>% filter(subgroup %in% c("ESBL")), kbayes$locus_rank, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# CP
coverage_cp_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups %>% filter(subgroup %in% c("CP")), kbayes_adj_coverage_subgroups %>% filter(subgroup %in% c("CP")), kbayes$locus_rank, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# South Asia
coverage_SAsia_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage %>% filter(subgroup=="South Asia"), kbayes_adj_coverage %>% filter(subgroup=="South Asia"), kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "")

# South Africa
coverage_SA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage %>% filter(subgroup=="Southern Africa"), kbayes_adj_coverage %>% filter(subgroup=="Southern Africa"), kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "")

# West Africa
coverage_WA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage %>% filter(subgroup=="Western Africa"), kbayes_adj_coverage %>% filter(subgroup=="Western Africa"), kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "")

# East Africa
coverage_EA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(kbayes_raw_coverage %>% filter(subgroup=="Eastern Africa"), kbayes_adj_coverage %>% filter(subgroup=="Eastern Africa"), kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "") 
```

``` {r}
(coverage_rawAdj_subgroups_top20global + ggtitle("All") +
   coverage_fatal_rawAdj_subgroups_top20global + ggtitle("Fatal") + coverage_esbl_rawAdj_subgroups_top20global + ggtitle("ESBL") + coverage_cp_rawAdj_subgroups_top20global + ggtitle("CP") + coverage_SAsia_rawAdj_subgroups_top20global + ggtitle("S. Asia") + coverage_EA_rawAdj_subgroups_top20global + ggtitle("E. Africa") + coverage_SA_rawAdj_subgroups_top20global + ggtitle("S. Africa") + coverage_WA_rawAdj_subgroups_top20global +ggtitle("W. Africa")) + plot_layout(guides="collect", nrow=2, ncol=4) & theme(legend.position = 'none')

ggsave("figures/AppendixFigS2.8_K_prevAdj_coverageRawAdj_top20global.pdf", width=12, height=8, device = cairo_pdf, family = "Arial Unicode MS")
ggsave("figures/AppendixFigS2.8_K_prevAdj_coverageRawAdj_top20global.png", width=12, height=8)
```
...
K_rank_SAs <- kbayes$region_est %>% ungroup %>% filter(subgroup=="South Asia") %>% arrange(-mean) %>% mutate(rank=row_number())
...
# add cluster-adjusted
# calculate global & regional coverage for these K - Fig 3bii
kbayes_adj_coverage_top8perRegion <- getCumulativeCoverageGlobalRegional(kbayes, k_rank_region8) %>%
  mutate(subgroup=fct_relevel(subgroup,c("Global", "South Asia", "Eastern Africa", "Southern Africa", "Western Africa")))

coverage_rawAdj_global_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion, kbayes_adj_coverage_top8perRegion, k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "")

#subgroups
kbayes_fatal_adj_min10perSite_coverage_top8perRegion <- getCumulativeCoverage(k_rank_region8, kbayes_fatal_adj_min10perSite$global_post)
kbayes_esbl_adj_min10perSite_coverage_top8perRegion <- getCumulativeCoverage(k_rank_region8, kbayes_esbl_adj_min10perSite$global_post)
kbayes_cp_adj_min10perSite_coverage_top8perRegion <- getCumulativeCoverage(k_rank_region8, kbayes_cp_adj_min10perSite$global_post)

kbayes_adj_coverage_subgroups_top8perRegion <- kbayes_esbl_adj_min10perSite_coverage_top8perRegion %>% mutate(subgroup="ESBL") %>%
  bind_rows(kbayes_cp_adj_min10perSite_coverage_top8perRegion %>% mutate(subgroup="CP")) %>%
  bind_rows(kbayes_fatal_adj_min10perSite_coverage_top8perRegion %>% mutate(subgroup="Fatal")) %>%
  bind_rows(kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="Global") %>% mutate(subgroup="All") ) %>%
  mutate(subgroup=fct_relevel(subgroup,c("All", "Fatal", "ESBL", "CP")))
...
ggsave("figures/Fig3_Kcoverage_raw.tiff", width=8, height=8)
```


# cluster-adjusted coverage top8K per region
``` {r}
# All
coverage_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("All")), kbayes_adj_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("All")), k_rank_region8, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# Fatal
coverage_fatal_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("Fatal")), kbayes_adj_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("Fatal")), k_rank_region8, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# ESBL
coverage_esbl_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("ESBL")), kbayes_adj_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("ESBL")), k_rank_region8, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# CP
coverage_cp_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("CP")), kbayes_adj_coverage_subgroups_top8perRegion %>% filter(subgroup %in% c("CP")), k_rank_region8, maxRank=20, xintercept=c(5,10,15,20), cols=group_cols, col_title = "", linetype_title="")

# South Asia
coverage_SAsia_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion %>% filter(subgroup=="South Asia"), kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="South Asia"), k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "")

# South Africa
coverage_SA_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion %>% filter(subgroup=="Southern Africa"), kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="Southern Africa"), k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "")

# West Africa
coverage_WA_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion %>% filter(subgroup=="Western Africa"), kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="Western Africa"), k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "")

# East Africa
coverage_EA_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion %>% filter(subgroup=="Eastern Africa"), kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="Eastern Africa"), k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "") 
```

``` {r}
(coverage_rawAdj_subgroups_top8perRegion + ggtitle("All") +
   coverage_fatal_rawAdj_subgroups_top8perRegion + ggtitle("Fatal") + coverage_esbl_rawAdj_subgroups_top8perRegion + ggtitle("ESBL") + coverage_cp_rawAdj_subgroups_top8perRegion + ggtitle("CP") + coverage_SAsia_rawAdj_subgroups_top8perRegion + ggtitle("S. Asia") + coverage_EA_rawAdj_subgroups_top8perRegion + ggtitle("E. Africa") + coverage_SA_rawAdj_subgroups_top8perRegion + ggtitle("S. Africa") + coverage_WA_rawAdj_subgroups_top8perRegion +ggtitle("W. Africa")) + plot_layout(guides="collect", nrow=2, ncol=4) & theme(legend.position = 'none')

ggsave("figures/AppendixFigS2.9_K_prevAdj_coverageRawAdj_top8perRegion.pdf", width=12, height=8, device = cairo_pdf, family = "Arial Unicode MS")
ggsave("figures/AppendixFigS2.9_K_prevAdj_coverageRawAdj_top8perRegion.png", width=12, height=8)
```
...
table2 <- kbayes_raw_coverage %>% mutate(cases="All") %>%
  ...
  mutate(summary=paste0(sprintf("%.1f", mean * 100), " (", sprintf("%.1f", lower * 100), "-", sprintf("%.1f", upper * 100), ")")) %>% 
...
# cluster adjusted
kbayes_fatal_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=kbayes$locus_rank, bayes=kbayes_fatal_adj_min10perSite, maxRank=20)

kbayes_esbl_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=kbayes$locus_rank, bayes=kbayes_esbl_adj_min10perSite, maxRank=20)

kbayes_cp_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=kbayes$locus_rank, bayes=kbayes_cp_adj_min10perSite, maxRank=20)

table2_adj <- kbayes_adj_coverage %>% mutate(cases="All") %>%
  bind_rows(kbayes_fatal_adj_min10perSite_coverageRegional %>% mutate(cases="  Fatal infections")) %>%
  bind_rows(kbayes_esbl_adj_min10perSite_coverageRegional %>% mutate(cases="  ESBL infections")) %>%
  bind_rows(kbayes_cp_adj_min10perSite_coverageRegional %>% mutate(cases="  CP infections")) %>%
  filter(rank %in% c(5,10,15,20)) %>% 
  mutate(upper=if_else(upper>1, 1, upper)) %>%
  mutate(summary=paste0(sprintf("%.1f", mean * 100), " (", sprintf("%.1f", lower * 100), "-", sprintf("%.1f", upper * 100), ")")) %>% 
  select(rank, subgroup, cases, summary) %>% 
...
# 
table2 <- bind_rows(table2 %>% mutate(Counts="raw"), table2_adj %>% mutate(Counts="adj"))
```

# add totals per subgroup
``` {r}
# cluster-adjusted
fatal_counts <- data_NNS_sites10 %>% filter(Mortality==1) %>% group_by(Region) %>% summarise(raw=n(), adj=length(unique(Cluster))) %>% pivot_longer(cols=c(raw, adj), names_to="Counts", values_to="n")
fatal_counts <- fatal_counts %>% group_by(Counts) %>% summarise(n=sum(n)) %>% mutate(Region="Global") %>% bind_rows(fatal_counts)

esbl_counts <- data_NNS_sites10 %>% filter(resistance_score>=1) %>% group_by(Region) %>% summarise(raw=n(), adj=length(unique(Cluster))) %>% pivot_longer(cols=c(raw, adj), names_to="Counts", values_to="n")
esbl_counts <- esbl_counts %>% group_by(Counts) %>% summarise(n=sum(n)) %>% mutate(Region="Global") %>% bind_rows(esbl_counts)

cp_counts <- data_NNS_sites10 %>% filter(resistance_score>=2) %>% group_by(Region) %>% summarise(raw=n(), adj=length(unique(Cluster))) %>% pivot_longer(cols=c(raw, adj), names_to="Counts", values_to="n")
cp_counts <- cp_counts %>% group_by(Counts) %>% summarise(n=sum(n)) %>% mutate(Region="Global") %>% bind_rows(cp_counts)

total_counts <- data_NNS_sites10 %>% group_by(Region) %>% summarise(raw=n(), adj=length(unique(Cluster))) %>% pivot_longer(cols=c(raw, adj), names_to="Counts", values_to="n")
total_counts <- total_counts %>% group_by(Counts) %>% summarise(n=sum(n)) %>% mutate(Region="Global") %>% bind_rows(total_counts)
...
table2 <- esbl_counts %>% mutate(cases="  ESBL infections") %>%
  ...
  full_join(table2, by=c("Region", "cases", "Counts")) %>%
...
table2$Region <- factor(table2$Region, levels = c("Global", "Eastern Africa", "Southern Africa", "Western Africa", "South Asia"))
table2$Counts <- factor(table2$Counts, levels = c("raw", "adj"))

table2 <- table2 %>%
  arrange(Region, cases, Counts) %>%
  relocate(n, .after=cases) %>% 
  relocate(Counts, .after=cases) %>% 
...
# write out raw to main Table 2
write_tsv(table2 %>% select(-Counts), file="tables/TableS4_coverage_rawAdj_globalTop20K.tsv")
...
# cluster-adjusted
kbayes_fatal_adj_min10perSite_coverageRegional_top8perRegion <- getCumulativeCoverageGlobalRegional(ranks=k_rank_region8, bayes=kbayes_fatal_adj_min10perSite, maxRank=20)

kbayes_esbl_adj_min10perSite_coverageRegional_top8perRegion <- getCumulativeCoverageGlobalRegional(ranks=k_rank_region8, bayes=kbayes_esbl_adj_min10perSite, maxRank=20)

kbayes_cp_adj_min10perSite_coverageRegional_top8perRegion <- getCumulativeCoverageGlobalRegional(ranks=k_rank_region8, bayes=kbayes_cp_adj_min10perSite, maxRank=20)

tableS4_adj <- kbayes_adj_coverage_top8perRegion %>% mutate(cases="All") %>%
  bind_rows(kbayes_fatal_adj_min10perSite_coverageRegional_top8perRegion %>% mutate(cases="  Fatal infections")) %>%
  bind_rows(kbayes_esbl_adj_min10perSite_coverageRegional_top8perRegion %>% mutate(cases="  ESBL infections")) %>%
  bind_rows(kbayes_cp_adj_min10perSite_coverageRegional_top8perRegion %>% mutate(cases="  CP infections")) %>%
  mutate(upper=if_else(upper>1, 1, upper)) %>%
  filter(rank %in% c(5,10,15,20)) %>% 
  mutate(summary=paste0(sprintf("%.1f", mean * 100), " (", sprintf("%.1f", lower * 100), "-", sprintf("%.1f", upper * 100), ")")) %>%
  select(rank, subgroup, cases, summary) %>% 
...
tableS4 <- bind_rows(tableS4 %>% mutate(Counts="raw"), tableS4_adj %>% mutate(Counts="adj"))
```
...
tableS4 <- esbl_counts %>% mutate(cases="  ESBL infections") %>%
  ...
  bind_rows(fatal_counts %>% mutate(cases="  Fatal infections")) %>%
  bind_rows(total_counts %>% mutate(cases="All")) %>%
  full_join(tableS4, by=c("Region", "cases", "Counts")) %>%
  mutate(cases=fct_relevel(cases,c("All", "  Fatal infections", "  ESBL infections", "  CP infections")))

# sort function isn't working when ordering region factor using fct_relevel
tableS4$Region <- factor(tableS4$Region, levels = c("Global", "Eastern Africa", "Southern Africa", "Western Africa", "South Asia"))
tableS4$Counts <- factor(tableS4$Counts, levels = c("raw", "adj"))
...
write_tsv(tableS4, file="tables/Table2_coverage_rawAdj_top8perRegion.tsv")
```
...
ggsave("figures/FigS4_KcoverageRaw_top20PerRegion.tiff", width=8, height=8)
```
...
# cluster-adjusted
obayes_adj_coverage <- getCumulativeCoverageGlobalRegional(obayes, obayes$locus_rank) %>%
  mutate(subgroup=fct_relevel(subgroup,c("Global", "South Asia", "Eastern Africa", "Southern Africa", "Western Africa")))

ocoverage_rawAdj_globalTop10_globalRegional <- plotCoverageRawAdj(obayes_raw_coverage, obayes_adj_coverage, obayes$locus_rank, maxRank=10, xintercept=c(4,8), cols=region_cols, col_title = "d) Region", linetype_title = "")

ocoverage_rawAdj_globalTop10_globalRegional

```


## O fatal - read posterior estimates for raw and adjusted counts
``` {r}
...
# cluster-adjusted
obayes_fatal_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/O_Fatal_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/O_Fatal_min10_adj_28_posterior_subgroup.csv.gz", fixOnames = T)

obayes_fatal_adj_min10perSite_coverage <- getCumulativeCoverage(obayes$locus_rank, obayes_fatal_adj_min10perSite$global_post) %>% mutate(type="min10")
```

## ESBL - read posterior estimates for raw and adjusted counts
``` {r}
...
# cluster-adjusted
obayes_esbl_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/O_ESBL_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/O_ESBL_min10_adj_28_posterior_subgroup.csv.gz", fixOnames = T)

obayes_esbl_adj_min10perSite_coverage <- getCumulativeCoverage(obayes$locus_rank, obayes_esbl_adj_min10perSite$global_post) %>% mutate(type="min10")
```


## CP - read posterior estimates for raw and adjusted counts
``` {r}
...
# cluster-adjusted
obayes_cp_adj_min10perSite <- parseModelledEstimates(global_post="outputs_core/O_Carba_min10_adj_28_posterior_global.csv.gz", region_post="outputs_core/O_Carba_min10_adj_28_posterior_subgroup.csv.gz", fixOnames = T)

obayes_cp_adj_min10perSite_coverage <- getCumulativeCoverage(obayes$locus_rank, obayes_cp_adj_min10perSite$global_post) %>% mutate(type="min10")
```
...
ggsave("figures/Fig4_O_prevAdj_coverageRaw.tiff", width=8, height=8)
```


# supp figure with O type coverage, raw and adjusted
``` {r}

obayes_adj_coverage_subgroups <- obayes_esbl_adj_min10perSite_coverage %>% mutate(subgroup="ESBL") %>%
  bind_rows(obayes_cp_adj_min10perSite_coverage %>% mutate(subgroup="CP")) %>%
  bind_rows(obayes_fatal_adj_min10perSite_coverage %>% mutate(subgroup="Fatal")) %>%
  bind_rows(obayes_adj_coverage %>% filter(subgroup=="Global") %>% mutate(subgroup="All") ) %>%
  mutate(subgroup=fct_relevel(subgroup,c("All", "Fatal", "ESBL", "CP")))

o_coverage_rawAdj_globalTop10_subgroups <- plotCoverageRawAdj(obayes_raw_coverage_subgroups, obayes_adj_coverage_subgroups, obayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=group_cols, col_title = "c) Subgroup", linetype_title="")

o_coverage_rawAdj_globalTop10_subgroups

# ESBL
esbl_ocov_rawAdj <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup %in% c("ESBL")), obayes_adj_coverage_subgroups %>% filter(subgroup %in% c("ESBL")), obayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=group_cols, col_title = "", linetype_title="")

# CP
cp_ocov_rawAdj <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup %in% c("CP")), obayes_adj_coverage_subgroups %>% filter(subgroup %in% c("CP")), obayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=group_cols, col_title = "", linetype_title="")

# All
ocov_rawAdj <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup %in% c("All")), obayes_adj_coverage_subgroups %>% filter(subgroup %in% c("All")), obayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=group_cols, col_title = "", linetype_title="")

# Fatal
fatal_ocov_rawAdj <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup %in% c("Fatal")), obayes_adj_coverage_subgroups %>% filter(subgroup %in% c("Fatal")), obayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=group_cols, col_title = "", linetype_title="")
...
``` {r}
# South Asia
ocoverage_SAsia_rawAdj_subgroups_top20global <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup=="South Asia"), obayes_adj_coverage_subgroups %>% filter(subgroup=="South Asia"), kbayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=region_cols, linetype_title = "")

# South Africa
ocoverage_SA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup=="Southern Africa"), obayes_adj_coverage_subgroups %>% filter(subgroup=="Southern Africa"), kbayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=region_cols, linetype_title = "")

# West Africa
ocoverage_WA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup=="Western Africa"), obayes_adj_coverage_subgroups %>% filter(subgroup=="Western Africa"), kbayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=region_cols, linetype_title = "")

# East Africa
ocoverage_EA_rawAdj_subgroups_top20global <- plotCoverageRawAdj(obayes_raw_coverage_subgroups %>% filter(subgroup=="Eastern Africa"), obayes_adj_coverage_subgroups %>% filter(subgroup=="Eastern Africa"), kbayes$locus_rank, maxRank=10, xintercept=c(5,10), cols=region_cols, linetype_title = "") 
```

``` {r}
(ocov_rawAdj + ggtitle("All") + fatal_ocov_rawAdj + ggtitle("Fatal") + esbl_ocov_rawAdj + ggtitle("ESBL") + cp_ocov_rawAdj + ggtitle("CP") + coverage_SAsia_rawAdj_subgroups_top20global + ggtitle("S. Asia") + coverage_SA_rawAdj_subgroups_top20global + ggtitle("S. Africa") + coverage_EA_rawAdj_subgroups_top20global + ggtitle("E. Africa") + coverage_WA_rawAdj_subgroups_top20global + ggtitle("W. Africa")) + plot_layout(guides="collect", axes="collect", nrow=2, ncol=4) & theme(legend.position = 'none')

ggsave("figures/AppendixFigS2.10_O_prevAdj_coverageRawAdj.pdf", width=12, height=8, device = cairo_pdf, family = "Arial Unicode MS")
ggsave("figures/AppendixFigS2.10_O_prevAdj_coverageRawAdj.png", width=12, height=8)
```
...
# STs with O4 or O2a in South Asia
data_NNS_sites10 %>% filter(Region=="South Asia" & O_genotype=="O4") %>% group_by(ST, K_locus)%>% count()
data_NNS_sites10 %>% filter(Region=="South Asia" & O_genotype=="O4") %>% group_by(ST, K_locus, Site)%>% count()

data_NNS_sites10 %>% filter(Region=="South Asia" & O_genotype=="O2a") %>% group_by(ST, K_locus)%>% count()
data_NNS_sites10 %>% filter(Region=="South Asia" & O_genotype=="O2a") %>% group_by(ST, K_locus, Site)%>% count()
...
obayes_fatal_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=obayes$locus_rank, bayes=obayes_fatal_adj_min10perSite, maxRank=10)

obayes_esbl_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=obayes$locus_rank, bayes=obayes_esbl_adj_min10perSite, maxRank=10)

obayes_cp_adj_min10perSite_coverageRegional <- getCumulativeCoverageGlobalRegional(ranks=obayes$locus_rank, bayes=obayes_cp_adj_min10perSite, maxRank=10)

tableS5_adj <- obayes_adj_coverage %>% filter(rank <= 10) %>% mutate(cases="All") %>%
  bind_rows(obayes_fatal_adj_min10perSite_coverageRegional %>% mutate(cases="  Fatal infections")) %>%
  bind_rows(obayes_esbl_adj_min10perSite_coverageRegional %>% mutate(cases="  ESBL infections")) %>%
  bind_rows(obayes_cp_adj_min10perSite_coverageRegional %>% mutate(cases="  CP infections")) %>%
  mutate(upper=if_else(upper>1, 1, upper)) %>%
  mutate(summary=paste0(sprintf("%.1f", mean * 100), " (", sprintf("%.1f", lower * 100), "-", sprintf("%.1f", upper * 100), ")")) %>% 
  select(locus, subgroup, cases, summary) %>% 
  pivot_wider(names_from=locus, values_from=summary, id_cols=c(subgroup, cases)) %>%
...
tableS5 <- bind_rows(tableS5 %>% mutate(Counts="raw"), tableS5_adj %>% mutate(Counts="adj"))
...
tableS5 <- esbl_counts %>% mutate(cases="  ESBL infections") %>%
  ...
  bind_rows(fatal_counts %>% mutate(cases="  Fatal infections")) %>%
  bind_rows(total_counts %>% mutate(cases="All")) %>%
  full_join(tableS5, by=c("Region", "cases", "Counts")) %>%
  mutate(cases=fct_relevel(cases,c("All", "  Fatal infections", "  ESBL infections", "  CP infections")))

# sort function isn't working when ordering region factor using fct_relevel
tableS5$Region <- factor(tableS5$Region, levels = c("Global", "Eastern Africa", "Southern Africa", "Western Africa", "South Asia"))
tableS5$Counts <- factor(tableS5$Counts, levels = c("raw", "adj"))
...
write_tsv(tableS5, file="tables/TableS5_coverage_O.tsv")
```
...
loo_k_coverage_plot <- loo_k_coverage %>% 
  mutate(subgroup=fct_relevel(subgroup,rev(c("Global", "South Asia", "Eastern Africa", "Southern Africa", "Western Africa")))) %>%
