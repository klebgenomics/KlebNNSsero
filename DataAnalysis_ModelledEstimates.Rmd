...
coverage_globalTop20_globalRegional <- plotCoverageRawAdj(kbayes_raw_coverage, kbayes_adj_coverage, kbayes$locus_rank, maxRank=20, cols=region_cols, linetype_title = "")
...
coverage_globalTop20_subgroups <- plotCoverageRawAdj(kbayes_raw_coverage_subgroups, kbayes_adj_coverage_subgroups, kbayes$locus_rank, maxRank=20, cols=group_cols, linetype_title = "")
...
# South Asia
coverage_SAsia_rawAdj_subgroups_top8perRegion <- plotCoverageRawAdj(kbayes_raw_coverage_top8perRegion %>% filter(subgroup=="Southern Asia"), kbayes_adj_coverage_top8perRegion %>% filter(subgroup=="Southern Asia"), k_rank_region8, maxRank=nrow(k_rank_region8), cols=region_cols, linetype_title = "")
...
ggsave("figures/FigX_K_prevAdj_coverageRawAdj.pdf", width=12, height=8, device = cairo_pdf, family = "Arial Unicode MS")
ggsave("figures/FigX_K_prevAdj_coverageRawAdj.png", width=12, height=8)
```
...
write_tsv(table2 %>% filter(Counts=="raw") %>% select(-Counts), file="tables/Table2_coverage.tsv")

# write out cluster-adjusted to appendix
write_tsv(table2 %>% filter(Counts=="adj") %>% select(-Counts), file="tables/TableS4_coverage_adjusted.tsv")
```
...
``` {r}
(ocov_rawAdj + ggtitle("All") + fatal_ocov_rawAdj + ggtitle("Fatal") + esbl_ocov_rawAdj + ggtitle("ESBL") + cp_ocov_rawAdj + ggtitle("CP")) + plot_layout(guides="collect", axes="collect", nrow=1, ncol=4) & theme(legend.position = 'none')

ggsave("figures/FigX_O_prevAdj_coverageRawAdj.pdf", width=12, height=4, device = cairo_pdf, family = "Arial Unicode MS")
ggsave("figures/FigX_O_prevAdj_coverageRawAdj.png", width=12, height=4)
```
